...
router.post('/orders', async (req, res) => {
  ...
  try {
    ...
    RecData.PayTypeID.map((item) => {
      if (
        item.Amount !==
        FilteredData[0].PaymentType.filter(
          (item1) => item1.ConcessionType === item.PayType,
        )[0].Amount
      ) {
        wrongentry = true;
      }
    ...
    if (wrongentry) {
      ...
      let FAmount = 0;
      RecData.PayTypeID.map(
        (item) => (FAmount += item.Amount * item.RegisteredNumber),
      );
      const options = {
        amount: FAmount * 118,
        ...
        notes: {
          ...
          RegisterdPersonTotal:
            RecData.PayTypeID[0].RegisteredNumber +
            (RecData?.PayTypeID[1]?.RegisteredNumber || 0),
